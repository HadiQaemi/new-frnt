...
export async function getPaperServer(id: string) {
    try {
        const response = await fetch(`${REBORN_API_URL}/articles/get_article_by_id/?id=${id}`, {
            next: {
                revalidate: 3600,
            },
            headers: {
...
export async function getInsightServer() {
    try {
        const response = await fetch(`${REBORN_API_URL}/insight/get_research_insights/`, {
            next: {
                revalidate: 3600
            },
        });
